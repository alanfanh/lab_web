...
def delete_all(name):
    ...
    if request.method=='POST':
        all_id = request.post.getlist('box')
        print("all_id=",all_id)
        return redirect_back()
        ...
        if template_id == 1:
            ...
            for data in datas:
                if checkEmpty(data['资产编号']) or not checkType(data['资产编号']):
                    ...
                    return redirect(url_for('main.show',name=depot.name))
                if checkEmpty(data['品牌']) or not  checkType2(data['品牌']):
                    flash('失败:品牌不能为空，且只能输入数字、字母、中文和下划线,品牌=%s' %data['品牌'],'err')
                    return redirect(url_for('main.show',name=depot.name))
                if checkEmpty(data['规格型号']) or not  checkType2(data['规格型号']):
                    flash('失败:品牌不能为空，且只能输入数字、字母、中文和下划线,规格型号=%s' %data['规格型号'],'err')
                    return redirect(url_for('main.show',name=depot.name))
            ...
            for data in datas:
                ...
                if checkEmpty(data['品牌']) or not  checkType2(data['品牌']):
                    ...
                    return redirect(url_for('main.show',name=depot.name))
                if checkEmpty(data['规格型号']) or not  checkType2(data['规格型号']):
                    flash('失败:规格型号不能为空,且只能输入数字、字母、中文和下划线,规格型号=%s' %data['规格型号'],'err')
                    return redirect(url_for('main.show',name=depot.name))
...
def explore(name):
    ...
    if template_id == 1:
        ...
        print("cmps=",cmps)
        head = ['资产编号','品牌','规格型号','数量(套/个)','部门','组别','储位','上次校准时间','复校时间','状态','借用人','借用邮箱','借用数量','借用时间','备注','盘盈/盘亏']
        sql_key = ['assetnumber','product_name','model_name','numbers','department','owner','position','calibratetime','resumptiontime','status','username','usermail','lend_numbers','lend_time','remark','profit_loss']
        content = [[cmp.assetnumber,cmp.product_name,cmp.model_name,cmp.numbers,cmp.department,cmp.owner,cmp.position,cmp.calibratetime.strftime("%Y-%m-%d"),cmp.resumptiontime.strftime("%Y-%m-%d"),status_dict[cmp.status],cmp.username,cmp.usermail,cmp.lend_numbers,cmp.lend_time.strftime("%Y-%m-%d"),cmp.remark,cmp.profit_loss] for cmp in cmps]
    data.append(head)
    print("data=",data)
    data.extend(content)
    print("********data=",data) 
